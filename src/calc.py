...
class BigData(object):
    ...
    def l_element_whe_want(self):
        ...
        if self.d_arguments["--all_children"] and not l_ele == ["*"]:
            l_ele = list(set(l_ele + [a for ele in l_ele for a in self.d_formula[ele]]))
    ...
    def d_l_element(self):
        ...
        assert d_run_id_ele, "No run have any element you request: {0}".format(self.l_element_whe_want)
    ...
    def d_ae_deviation(self):
        ...
        assert self.run_id_ref in self.l_run_id, "{0} need to be in {1}".format(self.run_id_ref,self.l_run_id)

        for run_id,d_ae_run_id in self.d_ae.items():
    
            for ele, energy, energy_ref in zipdic(d_ae_run_id,self.d_ae[self.run_id_ref]):
...
if __name__ == '__main__':
    d_arguments={"--run_id":[1,8],"--ele":["MnCl"],"--all_children":True}
